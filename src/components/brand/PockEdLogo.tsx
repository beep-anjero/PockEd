 ...
 *
 * The component is purely inline SVG so it scales crisply. The mark
 * keeps its brand colors. The wordmark uses `currentColor` so the
 * surrounding text color drives the "Pock" portion — pass `onDark`
 * to flip it to a light color, or set `text-[var(--foreground)]` on
 * the parent for automatic theme awareness.
 */
export function PockEdLogo({
  ...
  return (
    ...
    >
      ...
      >
        <MarkSvg gradId={gradId} size={h} />
      </span>
      <span
        className="flex flex-col justify-center min-w-0"
        style={{ width: w - h - 12, height: h }}
      >
        <WordmarkSvg gradId={gradId2} width={w - h - 12} height={h * 0.55} />
        <TaglineSvg width={w - h - 12} height={h * 0.22} />
      </span>
    </span>
  );
}

/** The brand mark (stacked cards + P + motion lines + sparkles). */
function MarkSvg({ gradId, size }: { gradId: string; size: number }) {
  return (
    <svg
      viewBox="0 0 64 64"
      width={size}
    ...
    >
      ...
      <path
        ...
        fill="#4F46E5"
      />
      <path d="M2 22H9" stroke="#4F46E5" strokeWidth="2" strokeLinecap="round" />
      <path d="M0 32H7" stroke="#4F46E5" strokeWidth="2" strokeLinecap="round" />
      <path d="M2 42H9" stroke="#4F46E5" strokeWidth="2" strokeLinecap="round" />
...
/** The wordmark "PockEd" — "Pock" uses currentColor so the parent
 *  text color drives it; "Ed" uses the brand gradient. */
  ...
  return (
    ...
    >
      ...
      <text
        ...
        fill="currentColor"
      >
        Pock<tspan fill={`url(#${gradId})`}>Ed</tspan>
      </text>
    </svg>
  );
}

/** The small "MICRO-LEARNING SPRINTS" tagline below the wordmark. */
function TaglineSvg({ width, height }: { width: number; height: number }) {
  return (
    <svg
      viewBox="0 0 240 12"
      width={width}
      height={height}
      xmlns="http://www.w3.org/2000/svg"
      style={{ display: 'block', marginTop: 2, color: '#4F46E5' }}
      preserveAspectRatio="xMinYMid meet"
    >
      <text
        x="0"
